...
function scrollElementToCenter(element) {
    ...
    const scrollOffset = elementCenterY - (window.innerHeight / 2);
    ...
    kreise.forEach(kreis => {
        kreis.addEventListener("click", function () {
            if (window.innerWidth > 768) {
                scrollElementToCenter(kreis);
            }

            const werkId = kreis.closest('.werk').id;
            zeigeWerkInfo(werkId);
        });
    });
});
...
function wechselbitte(){
    ...
    if (document.getElementById('ueberschrift').innerHTML === "Stahl mit Seele") {
		...
		document.getElementById('ueberschrift').innerHTML = "Steel with Soul";
		document.getElementById('ue1').innerHTML = "My Works";
		document.getElementById('th1').innerHTML = "News & Projects directly to your inbox";
		document.getElementById('newsletter-email').placeholder = "Your email address";
        document.getElementById('newsletter-button').innerHTML = "Subscribe";		
    } 
	else {
		selectedLanguage = 'de';
		document.querySelector(".übersetzung").classList.toggle("active");
        ...
        document.getElementById('ueberschrift').innerHTML = "Stahl mit Seele";
		document.getElementById('ue1').innerHTML = "Meine Werke";
		document.getElementById('th1').innerHTML = "Neuigkeiten & Werke direkt ins Postfach";
		document.getElementById('newsletter-email').placeholder = "Ihre E-Mail-Adresse";
        document.getElementById('newsletter-button').innerHTML = "Mitreisen";
	}
...
function zeigeWerkInfo(werkId) {
	...
	if (document.getElementById('ueberschrift').innerHTML === "Stahl mit Seele") {
		...
		description1 = "This work, the first in the &quot;Shovel&quot; series, addresses the merging of beings. The creation of a &quot;Hund-Wildschwein-Hybrid&quot; symbolizes the creative possibilities of mankind, but also raises critical ethical questions. The piece invites reflection on the responsibility that comes with such profound interventions in nature.";
	}
	...
	let description6;
	if (document.getElementById('ueberschrift').innerHTML === "Stahl mit Seele") {
		description6 = "&quot;Verrostetes Ideal&quot; zeigt, wie zerstörerisch der Einfluss verzerrter Schönheitsideale sein kann, wie sie in sozialen Medien ständig präsent sind. Die fragile Figur aus rostigem Stahl verkörpert eine junge Frau, deren Körper und Seele von diesen Idealen gezeichnet sind, ein Sinnbild für den inneren Kampf mit Anorexia nervosa. Was einst glänzte, ist nun vom Rost der Vergleiche und Selbstzweifel überzogen. Das Werk steht für all jene, die versuchen, einem perfekten Bild zu entsprechen, bis sie sich selbst darin verlieren. Es lädt dazu ein, innezuhalten und zu erkennen, dass wahre Schönheit nicht in der Anpassung an digitale Ideale liegt, sondern im Mut, echt zu sein.";
	}
	else {
		description6 = "&quot;Verrostetes Ideal&quot; reveals how destructive the influence of distorted beauty standards can be, ever present in the world of social media. The fragile figure made of rusted steel embodies a young woman whose body and soul have been scarred by these ideals, a symbol of the inner struggle with anorexia nervosa. What once shone brightly is now covered by the rust of comparison and self doubt. The artwork speaks for all those who strive to meet an image of perfection until they lose themselves in the process. It invites viewers to pause and recognize that true beauty is not found in conformity to digital ideals, but in the courage to be authentic.";
	}
